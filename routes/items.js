...
export async function handleItems(req, res, url) {
  ...
  if (req.method === "POST" && url.pathname === "/api/items") {
    const user = await requirePermission(req, res, PERMISSIONS.CREATE_ITEM);
    if (!user) return;
    ...
    const item = {
      ...
      logs: [{ at: new Date().toISOString(), step: "建档", note: "创建道具（" + user.displayName + "）" }]
    };
  ...
  if (patch && req.method === "PATCH") {
    const user = await requirePermission(req, res, PERMISSIONS.UPDATE_ITEM_STATUS);
    if (!user) return;
    const item = db.items.find(x => x.id === patch[1] || x.code === patch[1]);
    if (!item) return send(res, 404, { error: "item_not_found" });
    Object.assign(item, await body(req));
    item.logs ||= [];
    item.logs.push({ at: new Date().toISOString(), step: "状态", note: "更新为" + item.status + "（" + user.displayName + "）" });
    await saveDb(db);
  ...
  if (log && req.method === "POST") {
    ...
    const input = await body(req);
    item.logs ||= [];
    item.logs.push({ at: new Date().toISOString(), step: input.step || "记录", note: (input.note || "") + "（" + user.displayName + "）" });
    await saveDb(db);
    return send(res, 201, item);
  }
  ...
  if (action && req.method === "POST") {
    const user = await requirePermission(req, res, PERMISSIONS.BORROW_ITEM);
    if (!user) return;
    ...
    item.status = "已借出";
    item.logs.push({ at: new Date().toISOString(), step: "借用", note: (input.eventName || "演示") + " · " + (input.borrower || "") + "（" + user.displayName + "）" });
    await saveDb(db);
    return send(res, 201, item);
  }
